      ...
      <div class="attrListScroll">
        <div class="attr">
          <div v-for="(attrtype,i) in good.skus" :key="i">
            <div v-show="attrtype.quantity!=0" class="colorList" :class="{'select': selectSku==attrtype.skuid}" @click="changetype(attrtype.outerCode); selectSku=attrtype.skuid;good.skuid=attrtype.skuid; good.property=attrtype.property; good.outerCode=attrtype.outerCode; good.quantity=attrtype.quantity">{{attrtype.property}}</div>
          </div>
        </div>
      </div>
      <div class="confirm">
        <div class='counter-container'>
          ...
          <input class='count-input' v-model='good.count' @input="checkNum(good)" />
...
export default {
  ...
  methods: {
    ...
    updatecacheCart: function() {
      ...
      let url = `/a/order/amOrderShopping/addShopping?__sid=${this.sid}&numIid=${newItem.id}&skuId=${newItem.skuid}&count=${newItem.count}&flag=0`
      this.$api.get(url)
      .then(res=>{
        if(res.code==200){
